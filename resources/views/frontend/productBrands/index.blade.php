        ...
        <div class="col-md-12">
            @can('product_brand_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.product-brands.create') }}">
                            ...
                            <tbody>
                                @foreach($productBrands as $key => $productBrand)
                                    <tr data-entry-id="{{ $productBrand->id }}">
                                        ...
                                        <td>
                                            <span style="display:none">{{ $productBrand->status ?? '' }}</span>
                                            <input type="checkbox" disabled="disabled" {{ $productBrand->status ? 'checked' : '' }}>
                                        </td>
                                        <td>
                                            @can('product_brand_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.product-brands.show', $productBrand->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('product_brand_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.product-brands.edit', $productBrand->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('product_brand_delete')
                                                <form action="{{ route('frontend.product-brands.destroy', $productBrand->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('product_brand_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.product-brands.massDestroy') }}",
