...
@can('sales_order_detail_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.sales-order-details.create') }}">
                        ...
                        <tr data-entry-id="{{ $salesOrderDetail->id }}">
                            ...
                            <td>
                                @can('sales_order_detail_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.sales-order-details.show', $salesOrderDetail->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('sales_order_detail_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.sales-order-details.edit', $salesOrderDetail->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('sales_order_detail_delete')
                                    <form action="{{ route('admin.sales-order-details.destroy', $salesOrderDetail->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('sales_order_detail_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.sales-order-details.massDestroy') }}",
