    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.sales-orders.update", [$salesOrder->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date') ? 'is-invalid' : '' }}" type="text" name="date" id="date" value="{{ old('date', $salesOrder->date) }}" required>
                @if($errors->has('date'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('sales_person') ? 'is-invalid' : '' }}" name="sales_person_id" id="sales_person_id" required>
                    @foreach($sales_people as $id => $entry)
                        <option value="{{ $id }}" {{ (old('sales_person_id') ? old('sales_person_id') : $salesOrder->sales_person->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
